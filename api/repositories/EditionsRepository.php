...
class EditionsRepository extends Model
{
    ...
    {
        $sql = "SELECT id, location, start_date AS 'startDate', end_date AS 'endDate' FROM {$this->table} WHERE id = :id AND is_active = 1";
    ...
    {
        $sql = "SELECT id, location, start_date AS 'startDate', end_date AS 'endDate' FROM {$this->table} WHERE (CAST(start_date AS CHAR) LIKE :search OR location LIKE :search) AND is_active = 1 ORDER BY id ASC";
        $stmt = $this->db->prepare($sql);
